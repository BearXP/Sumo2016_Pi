...
class Sequencer(object):
  ...
  def get_sound(self):
    new_list=range(5)
    #new_list = new_list.remove(self.prev_sound)
    #new_list = list(new_list).remove(self.prev_sound)
    new_list = list(new_list[:self.prev_sound-1])+list(new_list[self.prev_sound:])
...
class Score(object):
  ...
  def set_image(self):
    ...
    for x in range(self.places):#number of places on the score
    ...
    conc = str(int(self.concecutive))
    conc = conc[max(0,len(conc)-3):]
    conc = "0"*(3-len(conc))+conc
    for x in range(3):#3 being the number of places on the streak
      place=10**(3-x)
      if( conc[x] != '.' ):
          index=int(conc[x])
